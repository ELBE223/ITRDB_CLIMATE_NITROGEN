###############################################################################
# 01b_build_BAI_base_global.R
#
# Description:
#   Parse site metadata (Lat, Lon, Species) from NOAA text headers.
#   Link metadata to BOTH local RWL file types from step 01a:
#     - tucson_rwl   (e.g., alge001.rwl)
#     - noaa_rwl     (e.g., alge001-noaa.rwl)
#
# Input:
#   - output/01a_download_ITRDB_RWL_global/tables/01a_ITRDB_download_status.csv
#
# Output:
#   - output/01b_build_BAI_base_global/tables/01b_ITRDB_site_metadata.csv
###############################################################################
...
# Input from 01a
input_script_name <- "01a_download_ITRDB_RWL_global"
input_dir         <- here::here("output", input_script_name)
input_index_path  <- path(input_dir, "tables", "01a_ITRDB_download_status.csv")
input_rwl_root    <- path(input_dir, "data", "rwl")

# Output for 01b
current_script_name <- "01b_build_BAI_base_global"
...
if (!file_exists(input_index_path)) {
  stop("Input index not found. Please run 01a_download_ITRDB_RWL_global.R first.")
}
...
# Save to CSV
output_file <- path(dir_tables, "01b_ITRDB_site_metadata.csv")
