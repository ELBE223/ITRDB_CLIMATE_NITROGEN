...
#
# Structure matches step 00a (Data Acquisition):
#   output/00a_download_ITRDB_RWL_global/
#       ├── data/
#       │    └── rwl/        (Raw .rwl files, organized by region)
#       ├── tables/          (CSV indices of downloaded files for logging)
...
pacman::p_load(
  ...
  readr,     # Reading/Writing CSV
  purrr      # Functional programming
)

options(timeout = 600)  # Allow for slow connections

# Parallel setup (Leave one core free)
...
get_region_file_list <- function(region) {
  ...
  data.frame(
    ...
    # Saves to: output/00a_.../data/rwl/europe/filename.rwl
...
# Save master index (important for step 01a later)
write_csv(all_files_df, path(dir_tables, "00a_ITRDB_master_index.csv"))

# --- 5) Parallel Download ----------------------------------------------------

cat("--- Step 5: Starting parallel download of .rwl files ---\n")

# Ensure region subfolders exist locally
...
# Run download using furrr (parallel)
...
# Save status log
final_index_path <- path(dir_tables, "00a_ITRDB_download_status.csv")
write_csv(all_files_df, final_index_path)

cat("\nProcessing complete.\n")
cat("Output saved to: ", base_out_dir, "\n")
cat("Please proceed to script 00b or 01a.\n")
