...
#
# Structure matches step 00b (Data Acquisition):
#   output/00b_download_ISIMIP_CRU/
#       ├── data/
#       │    └── climate/
#       │         ├── CRU_TS_v4.09/
#       │         └── ISIMIP3a/
...
# Set timeout for large file downloads (1 hour)
options(timeout = max(3600, getOption("timeout")))

# --- 2) Directories & Paths --------------------------------------------------

script_name  <- "00b_download_ISIMIP_CRU"
...
cat("=== 00b Download Climate and N-deposition ===\n\n")
cat("Output Root:  ", base_out_dir, "\n")
...
safe_download <- function(url, dest, tries = 3, sleep_sec = 5) {
  ...
  for (k in seq_len(tries)) {
    ...
    # Check if file exists and has content (> 100KB to be safe)
...
for (scen in isimip_ndep_scenarios) {
  ...
  for (var in isimip_ndep_vars) {
    ...
    for (rng in isimip_ndep_year_blocks) {
      ...
      # Structure: data/climate/ISIMIP3a/ndep/histsoc/ndep-nhx/file.nc
...
# Save summaries with new naming convention
write_csv(cru_downloads, path(dir_tables, "00b_CRU_download_summary.csv"))
write_csv(ndep_downloads, path(dir_tables, "00b_ISIMIP3a_download_summary.csv"))
...
write_csv(all_files, path(dir_tables, "00b_file_inventory.csv"))

cat("=== OUTPUT FILES ===\n")
cat("  CRU summary:      ", path(dir_tables, "00b_CRU_download_summary.csv"), "\n")
cat("  ISIMIP3a summary: ", path(dir_tables, "00b_ISIMIP3a_download_summary.csv"), "\n")
cat("  File inventory:   ", path(dir_tables, "00b_file_inventory.csv"), "\n")
cat("  Climate data dir: ", clim_dir, "\n")

cat("\n=== 00b Download Climate and N-deposition COMPLETE ===\n")